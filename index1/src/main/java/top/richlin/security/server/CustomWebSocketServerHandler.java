...
@Component
public class CustomWebSocketServerHandler{
     ...
     *
     *
     * 每次页面建立连接时传入到服务端的id，比如用户id等。可以自定义。
...
//        sid = SecurityContextHolder.getContext().getAuthentication().getName();
...
//        broadcast("连接成功");
    ...
    public void onMessage(String message, Session session) throws JsonProcessingException {
         ...
         */
        broadcast(JSON.parseObject(message).getString("message"));
...
//        }
    ...
    private void broadcast(String message){
        onlineSessionClientMap.forEach((sid,toSession)->{
            toSession.getAsyncRemote().sendText(message);
        });
    }
